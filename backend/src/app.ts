...
export default async function startServe(randomPort: Boolean = false) {
  ...
  // Manju Studio - ComfyUI 桥接路由
  const comfyuiRoutes = (await import("@/routes/manju/comfyui")).default;
  app.use("/api/comfyui", comfyuiRoutes);

  // Manju Studio - 流程导航路由
  const processRoutes = (await import("@/routes/manju/process")).default;
  app.use("/api/process", processRoutes);

  // Manju Studio - 剧本管理路由
  const scriptRoutes = (await import("@/routes/manju/script")).default;
  app.use("/api/manju/script", scriptRoutes);

  // Manju Studio - 角色管理路由
  const characterRoutes = (await import("@/routes/manju/character")).default;
  app.use("/api/manju/character", characterRoutes);

  // Manju Studio - 场景管理路由
  const sceneRoutes = (await import("@/routes/manju/scene")).default;
  app.use("/api/manju/scene", sceneRoutes);

  // Manju Studio - 分镜管理路由
  const storyboardRoutes = (await import("@/routes/manju/storyboard")).default;
  app.use("/api/manju/storyboard", storyboardRoutes);

  // Manju Studio - 提示词生成路由
  const promptRoutes = (await import("@/routes/manju/prompt")).default;
  app.use("/api/manju/prompt", promptRoutes);

  // Manju Studio - 画风管理路由
  const styleGuideRoutes = (await import("@/routes/manju/styleGuide")).default;
  app.use("/api/manju/styleGuide", styleGuideRoutes);

  // Manju Studio - 导出服务路由
  const exportRoutes = (await import("@/routes/manju/export")).default;
  app.use("/api/manju/export", exportRoutes);

  // Manju Studio - AI Agent 集成路由
  const agentRoutes = (await import("@/routes/manju/agent")).default;
  app.use("/api/manju/agent", agentRoutes);
